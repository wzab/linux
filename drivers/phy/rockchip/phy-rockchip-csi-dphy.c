...
struct rockchip_dphy {
	...
	int (*stream_on)(struct rockchip_dphy *priv);
};
...
static void mipidphy1_wr_reg(struct rockchip_dphy *priv, unsigned char addr,
			     unsigned char data)
{
	/*
...
static int mipidphy_rx_stream_on(struct rockchip_dphy *priv)
{
	...
	int i, hsfreq = 0;
	unsigned int data_rate_mbps = config->hs_clk_rate;

	do_div(data_rate_mbps, 1000 * 1000);

	pr_info("%s: lanes %d - data_rate_mbps %u\n", __func__, config->lanes, data_rate_mbps);
...
static int mipidphy_txrx_stream_on(struct rockchip_dphy *priv)
{
	const struct dphy_drv_data *drv_data = priv->drv_data;
	const struct hsfreq_range *hsfreq_ranges = drv_data->hsfreq_ranges;
	struct phy_configure_opts_mipi_dphy *config = &priv->config;
